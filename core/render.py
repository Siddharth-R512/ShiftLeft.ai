...
def feature_to_gherkin(feature: Feature) -> str:
    ...
    for sc in feature.scenarios:
        sc_tags = list(sc.tags) + [a for a in sc.verifies]
        if sc_tags:
...
def feature_to_csv(feature: Feature) -> str:
    ...
    writer.writerow(["Test ID", "Title", "Verifies", "Tags", "Type", "Steps", "Expected Result"])
    for i, sc in enumerate(feature.scenarios, start=1):
        ...
        writer.writerow([f"TC-{i:03d}", sc.name, ", ".join(sc.verifies), " ".join(sc.tags), test_type, steps_text, expected_text])
